...
class VideoPreprocessorPipeline:
    ...
    def preprocess_batch(self, batch: Dict[str, torch.Tensor],
                         face_detector: Optional[object] = None) -> Dict[str, torch.Tensor]:
        return self.preprocessor.preprocess_batch(batch, face_detector)
    ...
    def visualize_preprocessing(self, image: Union[torch.Tensor, np.ndarray, Image.Image, str],
                                save_path: Optional[str] = None):
        """Visualise l'effet du prétraitement sur une image."""
        import matplotlib.pyplot as plt
        
        original = Image.open(image).convert('RGB') if isinstance(image, str) else image
        ...
        processed_np = self.denormalize(torch.from_numpy(processed_np).permute(2, 0, 1)).numpy().transpose(1, 2, 0)
